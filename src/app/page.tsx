...
export default function Home() {
  ...
  // Update the barChartData whenever data changes
  useEffect(() => {
    const newLabels = data.map(item => item.activity).filter(activity => activity); // Get non-empty activities
    const newData = data.map(item => item.totalTime || 0); // Assume totalTime is the value for the graph

    setBarChartData(prevData => ({
      labels: [...newLabels],
      datasets: [
        {
          ...prevData.datasets[0], // Keep existing dataset properties
          data: [...newData],
        },
      ],
    }));
  ...
  return (
    <section className="flex flex-col gap-2 w-full h-full py-2 px-12 overflow-hidden">
      ...
      <div className="flex w-full justify-between items-center border border-slate-300 p-4"> 
        <BarGraph data={barChartData}/>
        <PieGraph/>
      </div>

      <div className="flex flex-col mt-6 w-full gap-6 mb-6">
         <h4 className="text-[18px] text-center mb-4"> Activity Breakdown</h4>
